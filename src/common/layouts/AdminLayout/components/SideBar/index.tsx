...
export function SideBar() {
  ...
  return (
    <>
      {/* Backdrop for mobile */}
      {!isCollapsed && (
        <div className="fixed inset-0 bg-pinit-primary z-40 lg:hidden" onClick={toggleSidebar} />
      )}
      <aside
        className={cn(
      ...
      >
        ...
        {/* Navigation */}
        <nav className={cn('flex-1 py-6', isCollapsed ? 'px-2 group-hover:px-4' : 'px-4')}>
              ...
              <ul className="space-y-2">
                {Items.map(item => (
                  <li key={item.path}>
                    ...
                    >
                      {item.icon}
                      <span className={cn(isCollapsed ? 'hidden group-hover:inline' : 'inline')}>
                        {item.title}
                      </span>
                    </NavLink>
                  </li>
