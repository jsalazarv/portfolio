...
export function About() {
  ...
  useEffect(() => {
    const timers = [600, 1100, 1600, 2100, 2600, 3100, 3600, 4100, 4600, 5100, 5600].map(
      (delay, i) => setTimeout(() => setVisibleLines(i + 1), delay),
    );
  ...
  return (
    <div className="w-full -mt-8 -mx-4 md:-mx-8 px-0">
      ...
      <div className="relative mx-4 md:mx-8 mb-8">
        ...
        <div className="absolute top-0 left-0 w-6 h-6 border-t-2 border-l-2 border-muted-foreground/40 z-30 pointer-events-none" />
        <div className="absolute top-0 right-0 w-6 h-6 border-t-2 border-r-2 border-muted-foreground/40 z-30 pointer-events-none" />
        <div className="absolute bottom-0 left-0 w-6 h-6 border-b-2 border-l-2 border-muted-foreground/40 z-30 pointer-events-none" />
        <div className="absolute bottom-0 right-0 w-6 h-6 border-b-2 border-r-2 border-muted-foreground/40 z-30 pointer-events-none" />

        {/* Header bar */}
        <div className="flex items-center gap-2 px-4 py-2 bg-muted/60 border-b border-border font-mono text-xs backdrop-blur-sm">
        ...
        <div className="relative bg-background/50 flex flex-col font-mono text-sm">
          ...
          <div className="relative z-20 flex flex-col gap-3 px-6 pb-8 border-t border-border/40">
            {visibleLines >= 3 && (
              <div className="hud-line-in flex items-start gap-2 pt-6">
            ...
            {visibleLines >= 4 && (
              <div className="hud-line-in flex items-start gap-2">
                <span className="text-primary shrink-0 mt-0.5">›</span>
                <div className="border-l-2 border-primary pl-3 flex-1">
                  <span className="block text-[9px] uppercase tracking-[0.2em] text-muted-foreground/50 mb-1.5">
                    career::
                ...
                <div className="border-l-2 border-primary pl-3 flex-1">
                  ...
                  <div className="space-y-1.5">
                    {education.map((item, i) => (
                      <div key={item.institution} className="hud-line-in flex items-baseline gap-2 text-xs" style={{ animationDelay: `${i * 180}ms` }}>
                        <span className="text-muted-foreground/50 shrink-0 tabular-nums">
                          {String(i + 1).padStart(2, "0")}
                        </span>
                        <span className="text-foreground">{item.degree}</span>
                        <span className="text-muted-foreground">· {item.institution}</span>
                        <span className="text-muted-foreground/50">· {item.period}</span>
                      </div>
                    ))}
                  </div>
                </div>
